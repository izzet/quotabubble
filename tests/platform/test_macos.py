...
def test_enabling_launch_at_login_writes_and_bootstraps_agent(
    ...
    target = tmp_path / "LaunchAgents" / "dev.izzet.quotabubble.plist"
    ...
    assert payload["Label"] == "dev.izzet.quotabubble"
...
def test_unchanged_loaded_agent_is_not_restarted(monkeypatch, tmp_path: Path) -> None:
    target = tmp_path / "dev.izzet.quotabubble.plist"
...
def test_is_loaded_checks_launchctl_result(monkeypatch) -> None:
    ...
    assert macos._is_loaded("gui/501/dev.izzet.quotabubble") is True
...
def test_run_launchctl_reports_failures(monkeypatch, caplog) -> None:
    ...
    with caplog.at_level(logging.WARNING):
        assert macos._run_launchctl("bootstrap", "gui/501/dev.izzet.quotabubble") is False
...
def test_run_launchctl_reports_success(monkeypatch) -> None:
    ...
    assert macos._run_launchctl("bootstrap", "gui/501/dev.izzet.quotabubble") is True
